...
RegisterNetEvent('A7-spawn:client:openUI', function(value)
...
RegisterNetEvent('A7-houses:client:setHouseConfig', function(houseConfig)
    Houses = houseConfig
end)

RegisterNetEvent('A7-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        QBCore.Functions.TriggerCallback('A7-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
                    myHouses[#myHouses+1] = {
...
RegisterNUICallback('spawnplayerappartment2', function(data, cb)
    ...
    TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
    TriggerEvent('QBCore:Client:OnPlayerLoaded')
    TriggerServerEvent('A7-houses:server:SetInsideMeta', 0, false)
    TriggerServerEvent('A7-apartments:server:SetInsideMeta', 0, 0, false)
    TriggerEvent('A7-apartments:client:LastLocationHouse', Data, Data2)
    PostSpawnPlayer()
    cb('ok')
end)
...
RegisterNUICallback('spawnplayer', function(data, cb)
    ...
    if type == "current" then
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('A7-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('A7-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
        PostSpawnPlayer()
    elseif type == "house" then
        PreSpawnPlayer()
        TriggerEvent('A7-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
        TriggerEvent('QBCore:Client:OnPlayerLoaded')
        TriggerServerEvent('A7-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('A7-apartments:server:SetInsideMeta', 0, 0, false)
...
RegisterNetEvent('A7-spawn:client:OpenUIForSelectCoord', function()
