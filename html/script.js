...
$(document).on('click', '#IconClassStyle', function(evt){
    ...
    if (type !== "lab") {
        ...
        $.post('https://A7-spawn/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
...
$(document).on('click', '.GreenBTN', function(evt){
    ...
    if (Mainlocation !== null){
        ...
        if (MainspawnType == "apartment1") {
            $.post('https://A7-spawn/spawnplayerappartment1', JSON.stringify({
                spawnloc: Mainlocation,
                apartName: apartNames,
            }));
        } else if(MainspawnType !== "appartment"){
            $.post('https://A7-spawn/spawnplayer', JSON.stringify({
                spawnloc: Mainlocation,
                typeLoc: MainspawnType
            }));
        }else {
            $.post('https://A7-spawn/chooseAppa', JSON.stringify({
...
function CloseAddCoord() {
    $.post('https://A7-spawn/CloseAddCoord', JSON.stringify({}));
